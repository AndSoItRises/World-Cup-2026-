...
# NOTE: kept pure-ASCII on purpose so it parses identically under Windows
# PowerShell 5.1 (cp1252) and PowerShell 7.
...
if ($newResult -or $dirty) {
    ...
    if (git status --porcelain) {
        ...
        if ($LASTEXITCODE -eq 0) { L "pushed OK ($msg)" } else { L "WARN push failed (will retry next cycle)" }
